...
function parseBoolean(val) {
    for(var re=0;re<booleanRegExps.length; ++re) {
        ...
        if(val.match(rgx.t)) { return 1; }
        if(val.match(rgx.f)) { return 0; }
    }
    
}

function parseNop(val) { return val; }
...
var types = [
    {adapt:adaptPlain, pad:padRight, validates:isBoolean    , parse:parseBoolean                               }, // boolean  
    {adapt:adaptPlain, pad:padRight, validates:isInteger    , parse:parseNop                                   }, // integer
    {adapt:adaptPlain, pad:padRight, validates:isBigInteger , parse:parseNop                                   }, // bigint
    {adapt:adaptPlain, pad:padRight, validates:isNumeric    , parse:parseNop, useLength:true                   }, // numeric
    {adapt:adaptPlain, pad:padRight, validates:isDouble     , parse:parseNop                                   }, // double precision
    {adapt:adaptText , pad:padRight, validates:isDate       , parse:parseNop                                   }, // date
    {adapt:adaptText , pad:padRight, validates:isTimestamp  , parse:parseNop                                   }, // timestamp
    {adapt:adaptText , pad:padLeft , validates:isVarchar    , parse:parseNop, useLength:true, isTextColumn:true}, // character varying
];
...
function determineColumnTypes(info){
    //console.log("DCT in", info.columnsInfo)
    if(! info.opts.columns || ! info.opts.columns[0].type) {
        info.columnsInfo.forEach(function(columnInfo, columnIndex){
            ...
            //console.log("column", columnIndex, columnInfo.typeInfo)
        });        
    } else {
        info.columnsInfo.forEach(function(columnInfo, columnIndex){
            var mapTypeIndex;
    ...
    //console.log("DCT out", info.columnsInfo)
    return info;
}
...
function createInsertValues(info, rows) {
    ...
    rows.forEach(function(row, index){
        ...
        group.push(margin+"("+row.map(function(adaptedValue,columnIndex){
            ...
            //console.log("column", column.name, column.typeInfo)
            var parsedValue = column.typeInfo.parse(adaptedValue);
            var recoveredLength = 0;
            if(parsedValue.length>column.columnLength-owedLength){
                owedLength=parsedValue.length-(column.columnLength-owedLength);
                recoveredLength = column.columnLength-parsedValue.length;
            }else{
                recoveredLength = owedLength;
                owedLength = 0;
                if(parsedValue.length>column.columnLength-recoveredLength){
                    owedLength=parsedValue.length-(column.columnLength-recoveredLength);
                    recoveredLength=recoveredLength-owedLength;
                }
            }
            // if parsedValue is not a string, we can recover from that
            return column.typeInfo.pad(column.columnLength-recoveredLength, parsedValue.length ? parsedValue : [parsedValue]);
        }).join(', ')+')');
    });
    inserts.push(group);
    return inserts;
