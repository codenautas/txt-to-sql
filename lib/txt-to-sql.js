...
function compareBuffers(one, two) {
    ...
    for(var i=0; i<max; i++){
        if(one[i]!=two[i]){ throw new Error('utf8 check failed in position: '+i); }
    }
}
...
function processEncodingOptions(info) {
    return getEncoding(info.txt).then(function(encoding) {
        ...
        if(info.opts.inputEncoding==='ANSI') {
            ...
            compareBuffers(info.txt, new Buffer(info.decodedBuffer, info.opts.inputEncoding));
        } else if(info.opts.inputEncoding==='UTF8') {
            info.decodedBuffer = info.txt.toString('utf8');
            compareBuffers(info.txt, new Buffer(info.decodedBuffer, 'utf8'));
        } else {
            info.decodedBuffer = info.txt.toString('utf8');
        }
        return info;
