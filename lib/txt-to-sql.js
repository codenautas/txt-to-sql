...
function determinePrimaryKey(info) {
    if(info.opts.includePrimaryKey) {
        ...
        try{
            var columnsInKey = [];
            info.columnsInfo.forEach(function(col,index) { if(excludedFromPK.indexOf(index)==-1) { columnsInKey.push(index); }} );
            var combinedKeys=new Array(info.rows.length);
            columnsInKey.some(function(columnIndex) {
        ...
        if(excludedFromPK.length && (! info.primaryKey || ! info.primaryKey.length)) {
