...
function generateCreateScript(info){
    ...
    if(columnValues.length) {
        ...
        columnValues.forEach(function(column, index) {
            //console.log("col", column, "idx", index);
            var huk = hasUniqueKeys(column, info.table.length);
            if(huk && firstUK==-1) { firstUK = index; }
            uks.push(huk);
        });
        uks.forEach(function(uk, index) {
            if( (firstUK !== -1 && ! uk) || (firstUK === -1 && uk)) {
                primaryKeys.push(quote(info.columnNames[index]));
            }
        });
        if(firstUK !== -1) {
            primaryKeys.push(quote(info.columnNames[firstUK]));
        }
