...
var formatFunctions = {
    'unmodified' : function(objectName) { return quote(objectName); },
    'lowercased_names' : function(objectName) { return quote(objectName.toLowerCase()); },
...
function transformNames(info) {
    //info.tableName = info.format(info.tableName);
...
function generateCreateScript(info){
    var scriptLines = [];
    scriptLines.push("create table "+info.format(info.tableName)+" (");
...
function generateInsertScript(info){
    ...
    info.scripts.push({type:'insert', sql:
        "insert into "+info.format(info.tableName)+" ("+info.columnsInfo.map(function(columnInfo){
