...
function verifyInputParams(info){
    ...
    info.transform = function(objectName) { return info.outputEngine.quote.fun(formatFunctions[info.opts.columnNamesFormat](objectName)); };
...
function processOutputBuffer(info) {
    if(info.opts.outputEncoding !== 'UTF8') {
        //console.log("processOutputBuffer: DETECTED", info.inputEncodingDetected, "INPUT", info.opts.inputEncoding, "OUTPUT", info.opts.outputEncoding)
        info.scripts = info.scripts.map(function(script, index) {
            //var ansi = iconv.encode(script.sql, "win1252").toString('binary');
            var ansi = iconv.decode(new Buffer(script.sql, "binary"), "win1252");
            //console.log("processOutputBuffer["+index+"]: "+getEncodingSinc(ansi));
            return { type:script.type, sql:ansi };
        });
    }
    return info;
}
