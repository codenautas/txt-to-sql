...
var outputFormats = {
    'postgresql': {
        types:mapTypes(['integer','bigint','numeric','double precision','text']),
        quote:quoteDouble,
...
function verifyInputParams(info){
    ...
    if(! (info.opts.outputFormat in outputFormats)) {
        errors.push("inexistent output format '"+info.opts.outputFormat+"'");
    }
    throwIfErrors(errors);
    var outputFormat=outputFormats[info.opts.outputFormat];
    info.typePatterns = outputFormat.types;
    info.transform = function(objectName) { return outputFormat.quote(formatFunctions[info.opts.fieldFormat](objectName)); };
    info.nameColumn = outputFormat.nameColumn;
    return info;
}
