...
function verifyInputParams(info){
    ...
    info.dropTable = outputEngine.dropTable;
    return info;
}
...
function processEncodingOptions(info) {
    return getEncoding(info.txt).then(function(encoding) {
        ...
        if(info.opts.inputEncoding==='ANSI') {
            ...
            var bufferConv = new Buffer(info.decodedBuffer, 'utf8');
            // console.log("TXT", info.txt); console.log("CNV", bufferConv)
            var max = Math.max(bufferConv.length, info.txt.length)
            for(var i=0; i<max; i++){
                if(bufferConv[i]!=info.txt[i]){
                    throw new Error('utf8 check failed in position: '+i);
                    //return 'Dif '+name+' pos '+i+' '+bufferConv[i]+' != '+info.txt[i];
                }
            }
        } else {
            info.decodedBuffer = info.txt.toString('utf8');
...
function generateDropTable(info) {
    ...
    if(info.opts.addDropTable) {
        info.scripts.push({type:'drop table', sql: info.dropTable(info.formatedTableName)+';\n'});
    }
    return info;
}
