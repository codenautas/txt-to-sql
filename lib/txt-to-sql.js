...
function verifyInputParams(info){
    ...
    if(! (info.opts.outputEngine in engines)) {
        errors.push("inexistent output format '"+info.opts.outputEngine+"'");
    }
    throwIfErrors(errors);
    var outputEngine=engines[info.opts.outputEngine];
    info.typePatterns = outputEngine.types;
    info.transform = function(objectName) { return outputEngine.quote(formatFunctions[info.opts.columnNamesFormat](objectName)); };
